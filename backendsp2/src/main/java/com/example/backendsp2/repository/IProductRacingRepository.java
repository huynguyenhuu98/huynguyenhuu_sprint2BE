...
//    Page<ProductRacing> findProductRacing(Pageable pageable, @Param("nameSearch") String nameSearch);
    @Query(value = "SELECT p.id AS id, p.name_racing  AS nameRacing,p.quantity AS quantity,p.create_date , p.price AS price, i.image AS image \n" +
    ...
    @Query(value = "SELECT p.id AS id, p.name_racing AS nameRacing,p.quantity AS quantity , pt.name_type AS type, p.price AS price, i.image AS image \n" +
            " FROM product_racing AS p \n" +
